...
def ft_tqdm(lst: range) -> None:
    ...
    for i in range(total):
        if i == 0:
            ...
            est_end_time = int((total - i) * (elapsed_time / i))
            ...
            if elapsed_time_hour:
                bar_str_e += f"[{elapsed_time_hour:02}:{elapsed_time_min:02}:{elapsed_time_sec:02}<"
            else:
                bar_str_e += f"[{elapsed_time_min:02}:{elapsed_time_sec:02}<"
            if est_end_time_hour:
                bar_str_e += f"{est_end_time_hour:02}:{est_end_time_min:02}:{est_end_time_sec:02}, "
            else:
                bar_str_e += f"{est_end_time_min:02}:{est_end_time_sec:02}, "
            if spit < 1:
                bar_str_e += f"{1/spit:5.2f}it/s]"
            else:
                bar_str_e += f"{spit:5.2f}s/it]"
